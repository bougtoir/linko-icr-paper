...
SHORT_TITLE = "Information Contribution Ratio in meta-analysis"
...
def build_english(res: dict) -> list:
    ...
    add(
        (
            ...
            "space. We formalise this structural feature as the Information Contribution "
            "Ratio (ICR), define its estimand, give estimators computable from published "
    ...
    add(
        (
            ...
            "not been formalised. This paper does three things. First, it defines the Information "
            "Contribution Ratio (ICR), a descriptor of the share of a study's measured "
            "information carried by its endpoint, together with an explicit estimand and "
            "estimators for both published summary statistics and individual participant data "
    ...
    add(
        (
            ...
            "variables are counted. We therefore present ICR as a reproducible structural "
            "descriptor and a reporting adjunct, and we present the accompanying visualisation, "
    ...
    add(
        (
            "p",
            "We defined the Information Contribution Ratio, gave estimators computable from "
            "published tables and from IPD, and tested the natural hypothesis that dispersion in "
            "this quantity is associated with statistical heterogeneity. The hypothesis was not "
    ...
    add(
        (
            "p",
            "The Information Contribution Ratio is a reproducible descriptor of how much of a "
            "study's measured data is carried by its endpoint. It is computable from published "
            "baseline tables, it varies across real studies, and it can be displayed alongside "
            "effect estimates. On present evidence it is not a diagnostic of heterogeneity and "
            "not a criterion for deciding whether studies may be pooled, and its simplest form "
            "is sensitive to "
            "variable counting. We recommend that it be used, if at all, as a transparently "
            "defined reporting adjunct with a prespecified counting protocol.",
        )
    )
...
def build_japanese(res: dict) -> list:
    ...
    add(("title", "情報寄与比 (Information Contribution Ratio): メタ解析における"
                  "エンドポイントの情報的重みの再現可能な記述指標と、その診断能力の限界"))
    add(("p", "著者: 大西達輝"))
    add(("p", "所属: [所属]"))
    add(("pagebreak", None))
